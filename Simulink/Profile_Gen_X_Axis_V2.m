...
y3 = (C1*cos(1/(d2+d3)*pi*(n3-l2)) + E1)/2;
y3 = reshape(y3, [N3,1]);
...
% join x and y axis 
y = [y1; y2; y3; y5; y6];
n = [reshape(n1, [N1,1]); reshape(n2, [N2,1]); reshape(n3, [N3,1]); reshape(n5, [N5,1]); reshape(n6, [N6,1])];
x_profile_V2 = [n y];

% Export
save(filename,'x_profile_V2');
...
scatter(z_profile_V2(:,1),z_profile_V2(:,2))
scatter(y_profile_V2(:,1),y_profile_V2(:,2))
